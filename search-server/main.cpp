...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query,
        ...
        auto matched_documents = FindAllDocuments(query, term);
...
template <typename fucntion>
    vector<Document> FindAllDocuments(const Query& query, fucntion term) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto &[document_id, term_freq] : word_to_document_freqs_.at(word)) {
                if (term(document_id, documents_.at(document_id).status, documents_.at(document_id).rating)) {
